...
const NAVIGATION_LABELS = ['Monitor', 'Cache', 'Evaluations', 'Observability'] as const;
...
async function expectSharedRouteContract(page: Page): Promise<void> {
  ...
  await expect(page.locator('#primary-navigation a')).toHaveCount(
    NAVIGATION_LABELS.length,
  );
...
for (const viewport of COMPACT_VIEWPORTS) {
  test(`compact navigation is accessible at ${viewport.name}`, async ({ page }) => {
...
for (const viewport of EXPANDED_VIEWPORTS) {
  test(`expanded navigation is accessible at ${viewport.name}`, async ({ page }) => {
    await installApiFixtures(page);
    await page.setViewportSize(viewport);
    await openEvaluationWorkspace(page);
    ...
    await expect(page.getByRole('button', { name: 'Open primary menu' })).toBeHidden();
    const navigation = page.getByRole('navigation', {
      name: 'Primary navigation',
    });
    ...
    expect(
      navigationBox!.x + navigationBox!.width - (lastLinkBox!.x + lastLinkBox!.width),
    ...
    expect(
      headerRowBox!.x + headerRowBox!.width - (uptimeBox!.x + uptimeBox!.width),
    ).toBeLessThanOrEqual(1);
    await expectSharedRouteContract(page);
  });
}

test('legacy route replaces history and preserves query and hash', async ({ page }) => {
  await installApiFixtures(page);
...
test('compact menu closes on routing and expanded resize cleanup', async ({ page }) => {
  await installApiFixtures(page);
  await page.setViewportSize({ width: 820, height: 1180 });
  await openEvaluationWorkspace(page);
  ...
  await expect(page.getByRole('button', { name: 'Open primary menu' })).toHaveAttribute(
    'aria-expanded',
    'false',
  );
...
test('zoom-equivalent, increased-text, and long-label layouts do not overflow', async ({
  ...
  await openEvaluationWorkspace(page);
  await expect(page.getByRole('button', { name: 'Open primary menu' })).toBeVisible();
  await expectNoHorizontalOverflow(page);
  ...
  await page.setViewportSize({ width: 1024, height: 900 });
  await page.getByRole('link', { name: 'Evaluations' }).evaluate((element) => {
