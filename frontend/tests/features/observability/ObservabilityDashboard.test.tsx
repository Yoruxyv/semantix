...
describe('ObservabilityDashboard', () => {
  it('uses a skeleton before rendering live metrics', async () => {
    let resolveMetrics:
      ((value: Awaited<ReturnType<typeof getRuntimeMetrics>>) => void) | undefined;
    ...
    expect(screen.getByLabelText('Loading runtime metrics')).toBeTruthy();
    expect(document.querySelectorAll('[data-skeleton-runtime-metric]')).toHaveLength(
      12,
    );
    ...
    const cacheGrid = screen.getByText('Cache').closest('section')?.querySelector('dl');
  ...
  it('distinguishes diagnostics loading and initial failure', async () => {
    ...
    expect(screen.getByLabelText('Loading runtime diagnostics')).toBeTruthy();
    ...
    expect(await screen.findByText('Diagnostics backend unavailable')).toBeTruthy();
  ...
  it('deduplicates simultaneous consumers of runtime metrics', async () => {
    const request = deferred<Awaited<ReturnType<typeof getRuntimeMetrics>>>();
    ...
    expect(screen.getAllByLabelText('Loading runtime metrics')).toHaveLength(2);
