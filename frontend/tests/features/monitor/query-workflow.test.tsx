...
describe('dashboard correctness', () => {
  ...
  it('starts without simulated radar traces', async () => {
    ...
    expect(container.querySelectorAll('[data-testid="similarity-point"]')).toHaveLength(
      0,
    );
  });

  it('shows a cache-readings skeleton while initial workspace data loads', async () => {
    vi.mocked(getCacheStats).mockReturnValue(new Promise(() => undefined));
    vi.mocked(getCacheThreshold).mockReturnValue(new Promise(() => undefined));
    ...
    expect(await screen.findByLabelText('Loading cache readings')).toBeTruthy();
    expect(document.querySelectorAll('[data-skeleton-reading]')).toHaveLength(6);
    expect(document.querySelector('[data-skeleton-similarity-plot]')).toBeTruthy();
  });
  ...
  it('reserves response space while a query is running', async () => {
    ...
    expect(await screen.findByLabelText('Loading query response')).toBeTruthy();
    expect(document.querySelectorAll('[data-skeleton-evidence-metric]')).toHaveLength(
      7,
    );
  });
  ...
  it('shows and highlights trace evidence on hover and keyboard focus', () => {
    ...
    expect(
      pointGroup?.querySelector('[data-testid="similarity-point"]')?.getAttribute('r'),
    ).toBe('8');
  ...
  it('renders a nullable response similarity as n/a', () => {
    ...
    expect(similarityLabel.parentElement?.querySelector('dd')?.textContent).toBe('n/a');
  });
