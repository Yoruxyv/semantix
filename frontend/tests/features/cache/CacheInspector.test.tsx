...
import type { CacheEntryListParams, CacheEntryMetadata } from '@/features/cache/types';
...
function successfulPage(items: CacheEntryMetadata[], params: CacheEntryListParams) {
  return {
    ok: true as const,
    data: {
      items,
...
function renderInspector(onMutation = vi.fn(), initialEntry = '/cache') {
...
describe('CacheInspector', () => {
  ...
  it('renders a contextual loading skeleton before entries resolve', () => {
    vi.mocked(listCacheEntries).mockReturnValue(new Promise(() => undefined));

    renderInspector();

    expect(screen.getByLabelText('Loading cache entries')).toBeTruthy();
    expect(document.querySelectorAll('[data-skeleton-entry-metric]')).toHaveLength(12);
  });
  ...
  it('loads complete Markdown instead of rendering a sliced raw preview', async () => {
    ...
    expect(screen.getByText('const complete = true;').closest('pre')).not.toBeNull();
  ...
  it('confirms a single delete and refreshes the listing', async () => {
    ...
    expect(await screen.findByText('The cache is empty.')).toBeTruthy();
    expect(vi.mocked(listCacheEntries).mock.calls.length).toBeGreaterThan(1);
  });
  ...
  it('confirms clear-all and refreshes after the mutation', async () => {
    ...
    expect(screen.getByRole('group', { name: 'Confirm clear cache' })).toBeTruthy();
  ...
  it('aborts an entry-list request when its last consumer unmounts', async () => {
    const request = deferred<Awaited<ReturnType<typeof listCacheEntries>>>();
  ...
  it('invalidates entry lists after deletion without touching datasets', async () => {
    ...
    queryClient.setQueryData(otherKey, successfulPage([alphaEntry], otherParams).data);
  ...
  it('invalidates every entry list after a cache clear', async () => {
    ...
    queryClient.setQueryData(otherKey, successfulPage([betaEntry], otherParams).data);
    queryClient.setQueryData(detailKey, {
      ...betaEntry,
